...
class PostgresDriver {
  ...
  static FK_ACTIONS = ['NO ACTION', 'RESTRICT', 'CASCADE', 'SET NULL', 'SET DEFAULT'];

  static INDEX_METHODS = ['btree', 'hash', 'gin', 'gist', 'brin', 'spgist'];

  /** CHECK 제약 목록. NOT NULL 은 pg_constraint 에 없으므로 섞이지 않는다. */
  async listChecks(schema, table) {
    ...
    // 'CHECK ((expr))' 접두어를 벗겨 식만 보여준다.
    // NOT VALID / NO INHERIT 접미어가 붙어 나올 수 있다 — 그대로 두면 괄호 매칭이 깨진다.
    return r.map((x) => {
      const m = /^CHECK\s*\((.*)\)((?:\s+(?:NO INHERIT|NOT VALID))*)$/s.exec(String(x.definition).trim());
      return { name: x.name, expression: m ? m[1] + (m[2] || '') : x.definition };
    });
  }

  buildIndexDDL(schema, table, spec) {
    if (spec.action === 'drop') {
      // PG 는 DROP 에서 인덱스 이름을 스키마로 한정한다 (MySQL 의 ON table 문법이 아니다).
      return [`DROP INDEX ${this.qualify(schema, spec.name)};`];
    }
    const method = spec.method && spec.method !== 'btree' ? ` USING ${spec.method}` : '';
    if (spec.method && !PostgresDriver.INDEX_METHODS.includes(spec.method)) {
      throw new Error(`지원하지 않는 인덱스 방식: ${spec.method}`);
    }
    const cols = spec.columns.map((c) => this.quote(c)).join(', ');
    return [`CREATE ${spec.unique ? 'UNIQUE ' : ''}INDEX ${this.quote(spec.name)} ON ${this.qualify(schema, table)}${method} (${cols});`];
  }

  buildConstraintDDL(schema, table, spec) {
    const t = this.qualify(schema, table);
    if (spec.action === 'drop') {
      // PG 는 PK/UNIQUE/FK/CHECK 모두 DROP CONSTRAINT 하나로 지운다.
      return [`ALTER TABLE ${t} DROP CONSTRAINT ${this.quote(spec.name)};`];
    }
    const cols = (spec.columns ?? []).map((c) => this.quote(c)).join(', ');
    switch (spec.kind) {
      ...
      case 'FOREIGN KEY': {
        const check = (v) => {
          if (!v) return null;
          if (!PostgresDriver.FK_ACTIONS.includes(v)) throw new Error(`지원하지 않는 FK 동작: ${v}`);
          return v;
        };
  ...
  buildCreateTableDDL(schema, spec) {
    const lines = spec.columns.map((c) => {
      let s = `  ${this.quote(c.name)} ${c.dataType}`;
      if (c.autoIncrement) s += ' GENERATED BY DEFAULT AS IDENTITY';
      if (!c.nullable) s += ' NOT NULL';
    ...
    const out = [`CREATE TABLE ${this.qualify(schema, spec.name)} (\n${lines.join(',\n')}\n);`];
    // PG 는 주석을 인라인으로 못 쓴다 — 별도 문장으로 낸다 (트랜잭션 안에서 함께 롤백된다).
    if (spec.comment) out.push(`COMMENT ON TABLE ${this.qualify(schema, spec.name)} IS ${literal(spec.comment)};`);
    for (const c of spec.columns) {
      if (c.comment) out.push(`COMMENT ON COLUMN ${this.qualify(schema, spec.name)}.${this.quote(c.name)} IS ${literal(c.comment)};`);
    }
    return out;
  }

  async buildDropTableDDL(schema, table, kind) {
    // 트리는 'm'(구체화 뷰)·'f'(외부 테이블)를 뷰/테이블로 뭉뚱그려 보여 주므로
    // 여기서 실제 relkind 를 확인해 맞는 DROP 문을 만든다.
    let word = kind === 'view' ? 'VIEW' : 'TABLE';
    try {
      const r = await this.rows(
      ...
      const relkind = r[0]?.relkind;
      if (relkind === 'm') word = 'MATERIALIZED VIEW';
      else if (relkind === 'f') word = 'FOREIGN TABLE';
      else if (relkind === 'v') word = 'VIEW';
      else if (relkind === 'r' || relkind === 'p') word = 'TABLE';
    } catch (_) { /* 조회 실패면 트리가 아는 종류대로 */ }
    return [`DROP ${word} ${this.qualify(schema, table)};`];
  }
