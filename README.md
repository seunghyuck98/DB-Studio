...
| 좌측 트리그리드 | `접속 › DB › 스키마 › 테이블` (MySQL 은 DB=스키마이므로 스키마 계층 생략), 우클릭 메뉴에 재연결 포함 |
| 연결 유지 | TCP keepalive + 3분 하트비트 + 끊겼을 때 자동 재연결 |
...
- **타이핑하면** 트리에서 이름을 즉시 걸러 낸다. 데이터베이스·스키마·테이블 어느 계층이든
  이름이 맞으면 남고, 이미 펼쳐 둔 하위 항목이 맞아도 남는다.
  아직 펼치지 않아 안을 모르는 곳은 (맞는 게 하나도 없을 때만) 남겨 둬서 들어가 볼 수 있게 한다.
- **`Enter` 를 누르면** DB 에 직접 질의해 **기본적으로 전체를 뒤진다** — 이름·컬럼명·주석·스크립트를
  모두, 모든 스키마(MySQL 은 모든 데이터베이스)에서 찾는다. 펼치지 않은 곳도 함께 걸린다.
...
| 컬럼명 | 컬럼 이름 | 켜짐 |
| 주석 | 테이블·컬럼 주석 | 켜짐 |
| 스크립트 | 뷰 정의, 함수·프로시저 본문, 트리거 정의 | 켜짐 |
| 모든 스키마 | 접속 전체 (끄면 현재 스키마만) | 켜짐 |

`범위` 버튼에서 필요 없는 항목을 끄면 좁힐 수 있다. 좁혀진 상태면 버튼이 강조된다.
스키마가 매우 많은 서버에서는 전체 검색이 느릴 수 있으니 그때만 범위를 줄인다.

> PostgreSQL 의 `모든 스키마` 는 **접속한 데이터베이스 안의 모든 스키마**를 뜻한다.
> 다른 데이터베이스까지 보려면 재접속이 필요해서 대상에 넣지 않는다.
> MySQL 은 데이터베이스가 곧 스키마라서 서버 전체가 대상이 된다.
...
트리 보기에서는 노드별 비중이 배경 막대 길이로 보인다. 막대의 기준은 계획이 주는 값에 따라
자동으로 고른다 — 비용이 있으면 비용, 없으면 실측 시간, 그것도 없으면 읽은 행 수
(MariaDB 10.x 의 계획 JSON 에는 비용이 아예 없다). 예상 행 수와 실제 행 수가 10배 이상
어긋나거나 인덱스를 타지 못하는 접근(`ALL`)은 노란색으로 표시된다.

MySQL·MariaDB 의 계획 JSON 은 노드 종류마다 키 이름이 다르고 버전마다 새 키가 생기므로,
아는 키만 골라 읽지 않고 구조 전체를 훑어 트리로 만든다. 이름을 모르는 노드는 키 이름
그대로라도 보이고, 파생 테이블(`materialized`)과 UNION 안쪽 계획도 전부 펼쳐진다.
...
접속마다 전용 커넥션 하나를 계속 열어 두기 때문에, 방화벽·로드밸런서나 DB 의 유휴 타임아웃에
걸려 조용히 끊길 수 있다. 세 가지로 막는다.

1. **TCP keepalive** — 드라이버 수준에서 켜 두어 유휴 소켓이 중간 장비에 끊기지 않게 한다.
2. **3분 주기 하트비트** — 유휴 상태에서도 가볍게 찔러 본다 (쿼리 히스토리에는 남지 않는다).
   실행 중일 때는 끼어들지 않는다.
3. **자동 재연결** — 그래도 끊겼으면 다음 작업 때 알아서 다시 연결한다.
   조회·메타데이터·객체 검색은 조용히 다시 실행되고, 트리에서 아무것도 안 해도 복구된다.

트리에서 접속을 우클릭하면 **재연결**이 있다. 저장된 접속 정보를 그대로 쓰므로 비밀번호를 다시 묻지 않는다.
연결이 끊긴 상태면 접속 옆 점이 노란색이 되고 `연결 끊김` 으로 표시된다.

> **트랜잭션이 열려 있었다면** 그 내용은 서버에서 사라진다 (DB 가 연결과 함께 버린다).
> 이때는 조용히 재시도하지 않고 "확정되지 않은 변경 N건이 되돌아갔습니다" 라고 알려 준다.
> 재연결 메뉴를 누를 때도 미확정 변경이 있으면 먼저 확인한다.
